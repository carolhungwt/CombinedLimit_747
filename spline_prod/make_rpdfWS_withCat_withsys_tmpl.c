...
RooNCSpline_2D_fast* make_2D_spPDF(std::vector<RooRealVar*> projvars, vector<vector<double>> projvals, vector<int> ndim, RooWorkspace* w,  TString tag ,int r,int cate_vbf,int prod_cate,bool res, bool scale, TString updn);
...
// Construct 3D spline 
	...
	RooNCSplineCore* pdfr1Nominal = make_2D_spPDF(projvars, projvals, ndim, ws,tag,1,cate_vbf,prod_cate, /*res=*/false, /*scale=*/false, /*updn*/"");
        RooNCSplineCore* pdfr1scaleUp = make_2D_spPDF(projvars, projvals, ndim, ws,tag,1,cate_vbf,prod_cate, /*res=*/false, /*scale=*/true, /*updn*/"Up");
        RooNCSplineCore* pdfr1scaleDn = make_2D_spPDF(projvars, projvals, ndim, ws,tag,1,cate_vbf,prod_cate, /*res=*/false, /*scale=*/true, /*updn*/"Down");
        RooNCSplineCore* pdfr1resUp = make_2D_spPDF(projvars, projvals, ndim, ws,tag,1,cate_vbf,prod_cate, /*res=*/true, /*scale=*/false, /*updn*/"Up");
        RooNCSplineCore* pdfr1resDn = make_2D_spPDF(projvars, projvals, ndim, ws,tag,1,cate_vbf,prod_cate, /*res=*/true, /*scale=*/false, /*updn*/"Down");	


	vector<RooAbsReal*> vec_pdfs;
	TString curdir = ".";
	vec_pdfs.push_back(pdfr1Nominal);
	vec_pdfs.push_back(pdfr1scaleUp);
        vec_pdfs.push_back(pdfr1scaleDn);	
	vec_pdfs.push_back(pdfr1resUp);
        vec_pdfs.push_back(pdfr1resDn);
...
RooNCSpline_2D_fast* make_2D_spPDF(std::vector<RooRealVar*> projvars, vector<vector<double>> projvals, vector<int> ndim, RooWorkspace* w,  TString tag ,int r,int cate_vbf,int prod_cate,bool res, bool scale, TString updn){
	...
	if(res)		prod_tag += "_Reso";
	else if(scale)  prod_tag += "_Scale";
	prod_tag += updn;
//ggH_Scale2e2muUp
	if(scale){
		if(tag=="4e")  	  scalingfactor = 1.003;
		if(tag=="2e2mu")  scalingfactor = 1.0015;
		if(tag=="4mu")    scalingfactor = 1.001;
	}
	...
	w->var("<NotChosenVar>")->setConstant(true);
		for(int ix=0; ix<ndim[<varIndex>]; ix++){
			dim[0]=projvals.at(<varIndex>).at(ix)/scalingfactor;
	...
	const char* pdfname = Form("r1_%s_%s",tag.Data(),prod_tag.Data());
	spPDF->SetNameTitle(pdfname,pdfname);
	return spPDF;
}
