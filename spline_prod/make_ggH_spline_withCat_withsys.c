...
void do_make_ggH_spline_withCat(TString tag, int cat,int quad, TString workdir="./rpdfWS_withCat"){
	...
	RooAbsReal* temppdfr1Nominal, *temppdfr1ResUp, *temppdfr1ResDown, *temppdfr1ScaleUp, *temppdfr1ScaleDown;
	...
	TFile* ftmpl = new TFile("../TemplateBuilder/Normalized_signal_tmpl_version1.root");
...
//	TString workdir="./rpdfWS_withCat";
	...
	wo->Print();
	temppdfr1Nominal = (RooAbsReal*) wo->obj(Form("r1_%s%d_%s",tag.Data(),cat,prodname.Data()));		
	temppdfr1ResUp = (RooAbsReal*) wo->obj(Form("r1_%s%d_%s_ResUp",tag.Data(),cat,prodname.Data()));
	temppdfr1ResDown = (RooAbsReal*) wo->obj(Form("r1_%s%d_%s_ResDown",tag.Data(),cat,prodname.Data()));
	temppdfr1ScaleUp = (RooAbsReal*) wo->obj(Form("r1_%s%d_%s_ScaleUp",tag.Data(),cat,prodname.Data()));
	temppdfr1ScaleDown = (RooAbsReal*) wo->obj(Form("r1_%s%d_%s_ScaleDown",tag.Data(),cat,prodname.Data()));
	...
	RooRealVar* CMS_res = new RooRealVar(Form("CMS_res_%s",coeffname.Data()),Form("CMS_res_%s",coeffname.Data()),-10,10);
	RooRealVar* CMS_scale= new RooRealVar(Form("CMS_scale_%s",coeffname.Data()),Form("CMS_scale_%s",coeffname.Data()),-10,10);
	RooArgList funclist, coefflist;
	funclist.add(*temppdfr1Nominal);
	funclist.add(*temppdfr1ResUp);
	funclist.add(*temppdfr1ResDown);
	funclist.add(*temppdfr1ScaleUp);
...
//**************** Extract template from root file*************************
	...
	TH2F* th2ftmpl = (TH2F*) ftmpl->Get(Form("ggzz_sig_%s",tag.Data()));
...
//	RooProdPdf* tempprodpdf = new RooProdPdf("tempprodpdf","tempprodpdf",*temppdf, *tmplfuncpdf);
	...
	temppdf->SetNameTitle(Form("r1_%s",prodname.Data()),Form("r1_%s",prodname.Data()));	
...
void make_ggH_spline_withCat_withsys(TString tag, int cat,int quad, TString workdir="./rpdfWS_withCat"){
